...
from pacman_project1.game import Grid
import pacman_project1.game
import os
import random
...
class Layout:
    ...
    def fillWithType(self, layoutText, total, type):
        assert type == 'P' or type == 'G' or type == '.'

        def replace_char(string, c, index):
            c_arr = list(string)
            c_arr[index] = c
            return "".join(c_arr)

        count = sum([line.count(type) for line in layoutText])
        if count < total:
            for _ in range(total - count):
                while True:
                    x = random.randint(0, len(layoutText) - 1)
                    y = random.randint(0, len(layoutText[0]) - 1)
                    if layoutText[x][y] not in ['P', 'G', '%', '.']:
                        layoutText[x] = replace_char(layoutText[x], type, y)
                        break

    def getNumGhosts(self):
        return self.numGhosts

    def getNumPacmen(self):
        return self.numPacmen
