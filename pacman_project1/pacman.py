...
"""
from pacman_project1.game import GameStateData
from pacman_project1.game import Game
from pacman_project1.game import Directions
from pacman_project1.game import Actions
from pacman_project1.util import nearestPoint
from pacman_project1.util import manhattanDistance
from pacman_project1 import util, layout
...
#        GameState.explored.add(self)
    ...
    def generateSuccessor( self, agentIndex, action):
        ...
        if agentIndex < self.data.numPacmanAgents:  # Pacman is moving
            # TODO look into how eaten works - currenlty not current for ghosts
    ...
    def getPacmanStates( self ):
        return self.data.agentStates[:self.data.numPacmanAgents]

    def getPacmanPosition( self, agentIndex ):
        return self.data.agentStates[agentIndex].getPosition()

    def getPacmanPositions(self):
        return [self.getPacmanPosition(i) for i in range(self.data.numPacmanAgents)]

    def getNumPacmanAgents(self):
        return self.data.numPacmanAgents
    ...
    def initialize( self, layout, numPacmanAgents=1000, numGhostAgents=1000 ):
        """
        Creates an initial game state from a layout array (see layout.py).
        """
        self.data.initialize(layout, numPacmanAgents, numGhostAgents)
...
def readCommand( argv ):
    ...
    if options.layout == 'RANDOM':
        l = randomLayout().split('\n')
    elif options.layout.startswith('RANDOM'):
        l = randomLayout(seed=int(options.layout[6:])).split('\n')
    else:
        l = layout.getLayout( options.layout )
    if l is None: raise Exception("The layout " + options.layout + " cannot be found")
    args["layout"] = layout.Layout(l)

    # Choose a Pacman agent
    noKeyboard = options.gameToReplay == None and (options.textGraphics or options.quietGraphics)
    pacmanType = loadPacmanAgent(options.pacman, noKeyboard)
...
def loadPacmanAgent(pacman, nographics):
    module_name = ".".join(pacman.split('/'))
    module = __import__(module_name[:-3], fromlist=('createAgents'))
    return module.createAgents
...
def runGames( layout, pacmen, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30 ):
    import __main__
